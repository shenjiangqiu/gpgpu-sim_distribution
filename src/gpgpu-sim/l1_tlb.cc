...
#include<deque>
#define SJQDEBUG
l1_tlb::l1_tlb(l1_tlb_config config,std::shared_ptr<page_manager> tlb_page_manager):
m_config(config),
...
m_tag_arrays(config.n_sets*config.n_associate){
    for(int i=0;i<config.n_sets*config.n_associate;i++){
...
void l1_tlb_config::parse_option(option_parser_t opp){
    ...
    option_parser_register(opp,"-l1tlb_mshr_entries",option_dtype::OPT_UINT32,&n_mshr_entries,"the set associate","16");
    option_parser_register(opp,"-l1tlb_mshr_maxmerge",option_dtype::OPT_UINT32,&n_mshr_max_merge,"the set associate","8");
    option_parser_register(opp,"-l1tlb_response_queue_size",option_dtype::OPT_UINT32,&response_queue_size,"the set associate 0=unlimited","0");
    option_parser_register(opp,"-l1tlb_miss_queue_size",option_dtype::OPT_UINT32,&miss_queue_size,"the set associate 0=unlimited","0");
    option_parser_register(opp,"-l1tlb_page_size",option_dtype::OPT_UINT32,&m_page_size,"the tlb_line_size","4096");
...
tlb_result l1_tlb::access(mem_fetch* mf,unsigned time){
    ...
    else
    {
        ...
        for(;start<end;start++){
            ...
            }else{//any time find the valid line, get it!
...
void l1_tlb::cycle(){
    ...
    while(m_mshrs->access_ready()){//push all the ready access to response Queue
        m_response_queue.push_back(m_mshrs->next_access());
    }
    

}
void l1_tlb::fill(mem_fetch *mf, unsigned long long time){//in ldst cycle, will call fill(that's from icnt)
    auto addr = mf->get_addr();
    auto v_addr = m_page_manager->get_vir_addr(addr);
    auto m_page_size_log2 = m_config.m_page_size_log2;
    auto n_set = m_config.n_sets;
